...
if ! curl -sf --max-time 10 https://static.rust-lang.org/dist/channel-rust-nightly.toml > /dev/null; then
  ...
  echo "Your connection is slow or blocked (firewall, hotel/campus network, etc)."
  echo "The Rust install may take a long time, or fail with a timeout error."
  echo "If you see errors, try again later or switch Wi-Fi/network!"
  sleep 5
fi
...
if ! $SUCCESS; then
    ...
    echo "This is almost always a temporary network/CDN issue (slow, blocked, or unreliable connection)."
    echo "What to try:"
    ...
    echo ""
    echo "Advanced: You can try manually downloading:"
...
if ! command -v cargo >/dev/null 2>&1; then
    ...
    echo "Try closing and reopening your terminal, then rerun this script."
    exit 1
fi
