...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
    DELAY=$((attempt * 10))
    echo "₍𝄐⁀̲𝄐₎ 🔧 Installing Rust (attempt $attempt/$MAX_ATTEMPTS)…"
    if ./rustup-init.sh -y --default-toolchain nightly --no-modify-path >"$LOG_FILE" 2>&1; then
        SUCCESS=true
        break
    else
        echo "₍𝄐⁀̲𝄐₎ ❌ Attempt $attempt failed. Retrying in $DELAY seconds…"
        sleep $DELAY
    fi
done
...
if ! $SUCCESS; then
    echo ""
    echo "₍𝄐⁀̲𝄐₎ 🚨 Rust install failed after $MAX_ATTEMPTS attempts."
    echo "₍𝄐⁀̲𝄐₎ Most issues are temporary network errors."
    echo "₍𝄐⁀̲𝄐₎ Try running manually after a few minutes:"
    echo "    ./rustup-init.sh -y --default-toolchain nightly --no-modify-path"
    echo "₍𝄐⁀̲𝄐₎ Or re-download from https://sh.rustup.rs"
    echo "₍𝄐⁀̲𝄐₎ See 'rustup-install.log' for details."
    exit 1
fi
...
if ! command -v cargo >/dev/null 2>&1; then
    echo "₍𝄐⁀̲𝄐₎ ❌ Rust installed, but 'cargo' not found."
    echo "₍𝄐⁀̲𝄐₎ Try closing/reopening your terminal, then rerun this script."
    exit 1
fi

echo "₍𝄐⁀̲𝄐₎ ✅ Rust nightly installed and ready."
